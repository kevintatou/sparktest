      ...
      <nav className="flex-1 px-3 py-4">
        <div className="space-y-2">
          {NAVIGATION_ITEMS.map((item) => (
            <NavigationItemComponent key={item.name} item={item} isActive={isActiveRoute(pathname, item.href)} />
          ))}
        </div>
      </nav>

      {/* Create Section */}
      <div className="px-3 pb-4 border-t border-slate-200 dark:border-slate-700 pt-4" ref={dropdownRef}>
        <div className="space-y-2">
          ...
          <div className="group relative">
            ...
            <div className={getTooltipClasses(isCreateOpen, true)}>Create New</div>
          </div>
