...
function StatusBadge({ status }: { status: string }) {
  const config = StatusConfig[status as keyof typeof StatusConfig]
  if (!config) return <span className="px-2 py-1 text-xs font-medium bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-400 rounded-full">unknown</span>
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-3">
        {Array(4).fill(null).map((_, i) => (
          <div key={i} className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl shadow-sm p-4 animate-pulse">
            <div className="flex items-center justify-between">
        ...
        <div className="bg-white dark:bg-slate-800 border-2 border-dashed border-slate-300 dark:border-slate-600 rounded-xl p-12 text-center">
          <div className="flex flex-col items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-slate-400 dark:text-slate-500">Create a test definition and run it to see results here.</p>
            </div>
          </div>
        </div>
      ) : (
