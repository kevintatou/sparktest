...
function MockKubernetesLogsWithError({ showError, errorMessage }: { showError: boolean; errorMessage?: string }) {
  if (showError) {
    return (
      <div>
        <h2>Kubernetes Logs</h2>
        <div role="alert" style={{ backgroundColor: '#fef2f2', border: '1px solid #fecaca', padding: '12px', borderRadius: '8px' }}>
...
describe("Error Handling Patterns", () => {
  it("validates error boundary patterns", () => {
    ...
    errorStates.forEach((errorMessage, index) => {
      const { container } = render(<MockKubernetesLogsWithError showError={true} errorMessage={errorMessage} />)
